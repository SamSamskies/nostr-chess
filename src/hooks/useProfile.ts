...
export function useProfile(pubkey?: string | null) {
    const { pool, effectiveRelay } = useNostr();
    ...
    useEffect(() => {
        ...
        const fetchPromise = async () => {
            try {
                ...
                const metadata = await pool.get([effectiveRelay], {
                    kinds: [0],
                    authors: [pubkey],
                });
                ...
                const games = await pool.querySync([effectiveRelay], {
                    kinds: [CHESS_KIND],
                    '#p': [pubkey],
                });
